...
function figureOut(arr) {
    for (let i = 0; i < arr.length-1; i++) {
        ...
        for (let j = 0; j < arr[i].length; j++) {
            if (a[j] != ' ')
            if (a[j] == b[j] ||  a[j] == b[j - 1] || a[j] == b[j + 1])
                continue;
            else return a[j];
        }
    }
...
console.log(figureOut(["a  b  c", "a b  c ", " ba   c"]));
console.log(figureOut(["bc  x", " bcx ", " xbc "]));
console.log(figureOut(["abcd", "cabd"]));
console.log(figureOut(["xyz", "yxz", "zyx"]))
console.log(figureOut([" x y z ", " xy  z ", "yx   z ", " xy  z "]));
